...
class Notes extends MY_Controller
{
    private $css_js_array = array(
        'codeboxr_css' => array('super-admin'),
    ...
    public function new_note()
    {
        ...
        //echo "<pre>"; print_r($get_receiver_and_member_list);exit;
        $table_col_start = '';
        ...
        if (is_array($get_receiver_and_member_list)  and count($get_receiver_and_member_list) > 0) {
            ...
            foreach ($get_receiver_and_member_list as $list) {
                ...
                <tr>
                    ...
                    <td style="display: none">' . $list->group_name . '</td>
        ...
        if (($this->uri->segment(2) == 'new' && $note_id = $this->input->post('note_id')) || ($this->uri->segment(2) == 'edit' && $note_id = (int)$this->uri->segment(3))) {
            ...
            if ($note_sended_or_not == true) {
                ...
                $this->frm_val->set_rules('note_status', 'Status of Note', 'trim|required|xss_clean|max_length[300]|int');
                ...
                $data['note_public_preview_url'] = base_url( 'note/public-view/' . md5( $note_id ) . '/?content_only=enable' ); //http://cloudenotes.com/note/public-view/{note_public_id}/?content_only=enable
        ...
        if ($this->user_info->user_access_level == 5 or $this->user_info->user_access_level == '5') {
            ...
            if ($response_note_data == false) {
                ...
                if ($json_cnote_response_info = $this->input->post('note_response_json')) {
                    ...
                    $note_fields_id = new stdClass;
                    $note_fields = new stdClass;
                    foreach ($note_json as $index => $item_object) {
                        ...
                        if (is_object($item_object)) {
                            ...
                            foreach ($item_object as $indexi => $item_objecti) {
                                $note_fields->$indexi = $indexi;
                               // ($note_fields, $indexi);
                                $note_fields_id->$indexi = $item_objecti->group_id;
                            }
                        }
                   ...
                   // var_dump($note_fields);
                    $json_cnote_response_info = json_decode($json_cnote_response_info);
                    //var_dump($json_cnote_response_info);die();
                    ...
                    foreach ($json_cnote_response_info as $index => $item_object) {
                        ...
                        foreach ($item_object as $item_type_key => $item_value_object) {
                            ...
                            if (property_exists( $note_json->$index,$item_type_key)) {
                               // var_dump($note_json->$index->$item_type_key->group_id); var_dump( $item_value_object->group_id);die();
                                if ($note_json->$index->$item_type_key->group_id == $item_value_object->group_id) {
                                    ...
                                    if ($item_type_key == 'checkbox' or $item_type_key == 'radio' or $item_type_key == 'select' or $item_type_key == 'dropdown') {
                                        //var_dump($note_json->$index->$item_type_key->total_items); var_dump( $item_value_object->total_items);die();
                                        if (count($note_json->$index->$item_type_key->total_items) == count( $item_value_object->total_items)) {
                                            ...
                                            foreach ($note_json->$index->$item_type_key->total_items as $item_key => $item_array) {
                                               ...
                                               // var_dump($note_json->$index->$item_type_key->total_items->$item_key->item_checked); var_dump( $item_value_object->total_items->$item_key->item_checked);die();
                                            }
                                        } else {
                                    ...
                                    else if ($item_type_key == 'name') {

                                        $note_json->$index->$item_type_key->first_name = $item_value_object->first_name;
                                        $note_json->$index->$item_type_key->last_name = $item_value_object->last_name;
    ...
    public function note_public_reply_view($section_member_id, $note_public_view_id, $encode_email)
    {
        ...
        $data['number_of_page_break'] = $total_page_break;
        $data['note_json_form_options'] = $response_note_data->response_note_response_json;
        $data['draggable_item'] = preg_replace('/\s\s+/', '', (str_replace(array("\r\n", "\r", "\n"), "", file_get_contents(APPPATH . 'views/note_template/form.html'))));
        $data['public_preview'] = 'true';
        $ext_css_js_array = array(
            'codeboxr_css' => array('note'),
            'codeboxr_js' => array('bootstrap.wizard.min', 'notes')
        );
    ...
    public function note_public_view() {
        ...
        if( $this->user_info->user_access_level > 4 or !$note_public_id = $this->uri->segment( 3, false )) {
            redirect( base_url( 'error' ) );
        } else {
            $response_note_data = $this->note_model->get_note_json_for_public_preview( $note_public_id );

            if ($response_note_data == false) {
                show_404();exit();
            }

            $note_json = json_decode($response_note_data->note_json_form_options);
            $note_json->note_name = $response_note_data->note_name;
            $response_note_data->response_note_response_json = json_encode($note_json);
    ...
    public function view_note_consent()
    {
        $css_js = array('codeboxr_css' => array('bootstrap-switch'), 'codeboxr_js' => array('bootstrap-switch', 'jquery.smooth-scroll.min'));
        $data['login_user_data'] = $this->user_info;
        $this->load->model('note_model');
        ...
        $data['note_parent_student'] = $this->note_model->get_consent_note_parent_student($note_id);
        //echo "<pre>";print_r($data['note_result']);
        $this->_render_admin('note/view_note_consent', $data, $css_js);
    }

    public function view_note_non_consent()
    {
        $css_js = array('codeboxr_css' => array('bootstrap-switch'), 'codeboxr_js' => array('bootstrap-switch', 'jquery.smooth-scroll.min'));
        $data['login_user_data'] = $this->user_info;
        $this->load->model('note_model');
        ...
        $data['note_parent_student'] = $this->note_model->get_non_consent_note_parent_student($note_id);
        //echo "<pre>";print_r($data['note_result']);
        $this->_render_admin('note/view_note_non_consent', $data, $css_js);
    }

    public function view_note_not_replied()
    {
        $css_js = array('codeboxr_css' => array('bootstrap-switch'), 'codeboxr_js' => array('bootstrap-switch', 'jquery.smooth-scroll.min'));
        $data['login_user_data'] = $this->user_info;
        $this->load->model('note_model');
        ...
        $data['note_parent_student'] = $this->note_model->get_not_replied_note_parent_student($note_id);
        //echo "<pre>";print_r($data['note_result']);
        $this->_render_admin('note/view_note_not_replied', $data, $css_js);
    }

    public function post_ajax_del_note() {


        $this->load->model('note_model');
        $getDelNote = $this->input->post('deleteArr'); //$this->debug($getDelNote); exit();

        $result = $this->note_model->deleteNote($getDelNote);
