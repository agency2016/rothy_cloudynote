...
class DX_Auth
{
	...
	*/
	...
	function _set_session($data)
	{
		...
		$user = [
			...
			'DX_logged_in'					    => true,
            'CN_org_name'                       => $data->organisation_name,
            'CN_first_name'                     => $data->first_name,
            'CN_last_name'                      => $data->last_name,
            'CN_user_access_level'              => $data->user_access_level,
            'CN_user_email'                     => $data->email,
            'CN_user_email_hash'                => md5($data->email),
            'CN_user_package_code'              => $data->package_code,
            'CN_user_organisation_id'           => $data->organisation_id
		];
	...
	*/
	...
	function is_admin()
	{
		return ($this->ci->session->userdata('CN_user_access_level') <= '1') ? true : false;
	}
	...
	function login($login, $password, $remember = false)
	{
		...
		if ( ! empty($login) AND ! empty($password))
		{
			...
			{
                $data['auth_message'] = 'We already sent you an activation link to your email. Please check your email to activate your account.';
                $this->ci->session->set_flashdata('auth_message', $data['auth_message']);
			...
			{
                //exit('auth_login_username_not_exist');
				// Increase login attempts
				$this->_increase_login_attempt();
				// Set error message
	...
	function register($username = '', $password, $email, $register_as = '1', $invite_name_of_user = '')
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');

		$this->ci->load->helper( array( 'url', 'function' ) );
		...
		$new_user = array(			
			'username'				=> $username,
			'password'				=> md5($password),//crypt($this->_encode($password)),
            'email'		            => $email,
            'hashing_email'         => md5($email),
			'last_ip'               => $this->ci->input->ip_address(),
            'register_as'           => $register_as
		);

        if($invite_name_of_user != '' and is_array($invite_name_of_user)) {
            $new_user['first_name'] = $invite_name_of_user['fname'];
            $new_user['last_name'] = (!empty($invite_name_of_user['lname'])) ? $invite_name_of_user['lname'] : '';
        }



        if($this->ci->user_temp->check_email($new_user['email'])) {
           $this->_auth_error = $this->ci->lang->line('auth_username_or_email_exist_in_temp');
            return $result;
        }

        if($this->ci->users->check_email($new_user['email'])) {
           $this->_auth_error = $this->ci->lang->line('auth_username_or_email_exist_in_user');
            return $result;
        }
		...
		if ($insert)
		{
            ...
            $new_user['user_access_role_id'] = $this->ci->user_temp->get_user_access_level_id($new_user['register_as']);
			...
			if ($this->ci->config->item('DX_email_activation'))
			{
				...
				// Activation Link
                //base_url('activation/'.$new_user['activation_key']);
				//$new_user['activate_url'] = site_url($this->ci->config->item('DX_activate_uri')."{$new_user['username']}/{$new_user['activation_key']}");
				$new_user['activate_url'] = base_url('activation/'.$new_user['activation_key'].'/');
         ...
         *      'attachment' => (array|string)
                *  )*/
                ...
                $mail_sending_array['to']['email'] = $new_user['email'];
                $mail_sending_array['to']['password'] = $new_user['password'];
                $mail_sending_array['to']['active_url'] = base_url('activation/'.$new_user['activation_key']);
                $mail_sending_array['subject'] = 'Activate Your New Account';
                $mail_sending_array['message'] = $message;
                send_mail( $mail_sending_array);
		...
		if ($login)
		{
			...
			if ($query = $this->ci->users->get_login($login) AND $query->num_rows() == 1)
			{
				...
				if ( ! $row->newpass_key)
				{
					...
					$data['reset_password_uri'] = base_url('change-password/'.$data['key']);
                    ...
                    //$message = $this->ci->load->view('page/email/activation', $new_user, true);
	...
	{		
		...
		// Activate user
		if ($query = $this->ci->user_temp->activate_user($key) AND $query->num_rows() > 0)
		{
			// Get user 
			$row = $query->row_array();
			...
			if ($this->ci->users->create_user($row))
			{
                //exit($del);
				// Trigger event
                //I am commented this method for avoiding activation error issue--> Sudarshan
				//$this->user_activated( $this->ci->db->insert_id() );
